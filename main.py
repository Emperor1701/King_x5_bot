...
def owner_kb()->ReplyKeyboardMarkup:
    rows=[
        ...
        [KeyboardButton(text=BTN_TEST_HL), KeyboardButton(text=BTN_WIPE_ALL)],
...
ALL_BTN_TEXTS = {
    ...
    BTN_MERGE, BTN_EXPORT, BTN_IMPORT, BTN_LISTQUESTIONS, BTN_EDITQUESTION, BTN_TEST_HL
}
...
@dp.callback_query(
    ...
    F.data.regexp(r"^(addq|editq|delq|delqconfirm|briefdur|attach_mode|done:|skip:|wipe:|listq:|pickq:|editqs:|pickqs:|pub:|scoreq:)")
)
...
async def pick_quiz_for_question(cb:CallbackQuery, state:FSMContext):
    ...
    for q in qs[:80]:
        # show short preview
        label = f"Q{q['id']}"
        kb.button(text=label, callback_data=f"pickqs:{q['id']}")
    kb.adjust(6)
    await state.set_state(EditQStates.edit_text)
    await state.update_data(edit_question_pick_mode=1)
...
async def publish_quiz(cb:CallbackQuery):
    ...
    for q in qs:
        ...
        m = await bot.send_poll(
            ...
            is_anonymous=False,   # حتى يجي PollAnswer مع المستخدم
...
async def on_poll_answer(pa: PollAnswer):
    ...
    q_exec("""INSERT INTO quiz_responses(chat_id,quiz_id,question_id,user_id,username,option_index,is_correct,answered_at)
              VALUES (%s,%s,%s,%s,%s,%s,%s,%s)""",
           (chat_id,quiz_id,qid,u.id, u.username or u.full_name, chosen, is_ok, _now().isoformat()))

    # Announce to chat with username
...
async def show_score(cb:CallbackQuery):
    quiz_id=int(cb.data.split(":")[1])
    # total questions published for this quiz in this chat
...
# ---------- Simple HL helper ----------
@dp.message(F.text==BTN_TEST_HL)
async def test_hl_start(msg:Message):
    if not await ensure_owner(msg): return
    await msg.answer("أرسل نتيجتك بشكل <code>23/30</code> لحساب النسبة والمستوى.", reply_markup=owner_kb())

@dp.message(F.text.regexp(r"^\s*(\d{1,3})\s*/\s*(\d{1,3})\s*$"))
async def test_hl_calc(msg:Message):
    m = re.match(r"^\s*(\d{1,3})\s*/\s*(\d{1,3})\s*$", msg.text)
    ok = int(m.group(1)); total = max(int(m.group(2)),1)
    ok = min(ok, total)
    pct = round(100*ok/total, 1)
    lvl = "Unter A2"
    if pct >= 75: lvl = "B1"
    elif pct >= 60: lvl = "A2"
    await msg.reply(f"🎧📖 نتيجتك: <b>{ok}/{total}</b> ({pct}%)\n🎯 المستوى التقديري: <b>{lvl}</b>", reply_markup=owner_kb())

# ---------- Not-yet-implemented buttons ----------
@dp.message(F.text.in_({BTN_BUNDLES, BTN_MERGE, BTN_EXPORT, BTN_IMPORT}))
...
async def wipe_all_decide(cb:CallbackQuery, state:FSMContext):
    ...
    for tbl in ["options","question_attachments","questions","sent_polls","writing_submissions","brief_windows","quiz_responses","quizzes"]:
