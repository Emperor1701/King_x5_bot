...
# --- إعداد تأخيرات بسيطة لتخفيف Flood ---
...
@dp.callback_query(
    ...
    F.data.regexp(r"^(addq|editq|delq|delqconfirm|briefstop|done:|skip:|wipe:|listq:|pgql:|pgqs:|delpick:|del:|delc:|editqs:|pickqs:|qview:|pub:|pubdur:|scoreq:|scorepick:|scorerun:|export:|bund:|merge:|att:|attadd:|attdone:|editm:)")
)
...
def _questions_page(quiz_id: int, mode: str, page: int = 0):
    ...
    for r in chunk:
        ...
        if mode == "q_view":
            ...
            kb.button(text=f"✏️ {preview}", callback_data=f"pickqs:{r['id']}")
...
# ---------- مصحّح مجاني بسيط ----------
def free_b1_grade(text: str) -> Tuple[int, str, Dict]:
    raw = text.strip()
    t = raw.lower()
    ...
    lvl = "Unter A2" if total<=6 else ("A2" if total<=14 else "B1")
    ...
    if cohesion_score < 2: tips.append("استخدم روابط: weil, deshalb, außerdem, damit…")

    feedback = "جيد جدًا، استمرّ." if not tips else "؛ ".join(tips)
    return total, lvl, {"feedback": feedback}
...
# ---------- نشر الاختبار كـ Polls + جلسة نشر (run) ----------
    ...
    try:
        ...
        if answered == total_polls and total_polls > 0:
            ...
            await bot.send_message(
                chat_id,
                f"📊 نتيجة {mention_html(u.id, u.full_name or u.username or 'طالب')}: <b>{int(correct)}/{int(total_polls)}</b>",
...
async def score_pick_quiz(cb:CallbackQuery, state:FSMContext):
    ...
    for r in runs[:50]:
        ...
        kb.button(text=f"🕒 {dt} — {r['qcount']} س.", callback_data=f"scorerun:{r['id']}")
    kb.adjust(1)
