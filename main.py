...
def ensure_schema():
    ddls = [
        ...
        # NEW: جدول القفل (أنواع الوقت صحيحة TIMESTAMPTZ)
        ...
        # فهرس يضمن صف واحد فقط (اختياري بس مفيد)
        """CREATE UNIQUE INDEX IF NOT EXISTS bot_lock_one ON bot_lock((1))""",
        # NEW: كاش لعناوين المجموعات
...
def migrate_schema():
    ...
    # NEW: تأكد أن أعمدة bot_lock من نوع TIMESTAMPTZ (علاج الخطأ text <= timestamptz)
    try:
        q_exec("""
    ...
    # شيل الديفولت بعد التحويل حتى تضل الأعمدة NOT NULL بدون قيم افتراضية
    ...
    # NEW: فهرس القفل إذا ما كان موجود
    q_exec("CREATE UNIQUE INDEX IF NOT EXISTS bot_lock_one ON bot_lock((1))")

    # NEW: تأكد من وجود جدول الكاش (لو نسخ قديم ما فيه ensure حديث)
